...
export async function GET(request:NextRequest) {
  ...
  try {
    // Connect to MongoDB
    await connectDB();
    const items = await SimpleTrend.find({})
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate the data
    const validatedData = SimpleTrendSchemaVl.parse(json);

    // Connect to MongoDB
    await connectDB();
    // Delete if any data exist
    await SimpleTrend.deleteMany({})
    // Insert the validated data into MongoDB
    await SimpleTrend.create(validatedData);
...
export async function PUT(req: NextRequest) {
  ...
  if (checkSource === 'Authorized') { 
    ...
    try {
      ...
      SimpleTrend.updateOne({}, validatedData);
