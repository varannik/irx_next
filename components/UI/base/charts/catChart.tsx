...
const BarLabels = ({ values , forcastedValue }: { values: number[] , forcastedValue:string | null |undefined}) => {
  ...
  return (
    ...
    >
      <div className="absolute bottom-0 left-0 flex items-center text-xs text-text-active">-5%</div>
      {values.map((widthPercentage, index) => {
        ...
        return (
          ...
          >
            {showLabel ? (
              <span
                className={cx("block translate-x-1/2 text-sx tabular-nums")}
              >
                <div >
...
interface CategoryBarProps extends React.HTMLAttributes<HTMLDivElement> {
  values: number[]
  forcastedValue?: string  | null | undefined
  colors?: AvailableChartColorsKeys[]
  ...
  (
    ...
    return (
      <div
        ...
        {...props}
      >
        {showLabels ? <BarLabels values={values}  forcastedValue={forcastedValue}/> : null}
        <div className="relative flex h-2 w-full items-center">
          ...
          {marker !== undefined ? (
            ...
            >
              {marker.tooltip ? (
                <Tooltip side="bottom" defaultOpen={true} asChild content={marker.tooltip}>
                  <div
                    aria-hidden="true"
                    className={cx(
